...
class Measurer extends SingleChildRenderObjectWidget {
  ...
  const Measurer({super.key, this.onMeasure, this.onPaintBoundsChanged, required Widget super.child});
  ...
  RenderObject createRenderObject(BuildContext context) {
    return _MeasureSizeRenderObject(
      onMeasure: onMeasure ?? (_, __) {},
      onPaintBoundsChanged: onPaintBoundsChanged ?? (_) {},
    );
  }
}

/// When a [size] or its associated [constraints] changed.
typedef OnMeasure = void Function(Size? size, BoxConstraints? constraints);

/// When the onscreen [paintBounds] changed.
typedef OnPaintBoundsChanged = void Function(Rect? paintBounds);

/// An element that notifies whenever its size changes.
class _MeasureSizeRenderObject extends RenderProxyBox {
  _MeasureSizeRenderObject({this.onMeasure, this.onPaintBoundsChanged});
    ...
    if (measureChanged || paintBoundsChanged) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        if (paintBoundsChanged) {
          onPaintBoundsChanged?.call(_paintBounds);
        }
      });
    }
  }
